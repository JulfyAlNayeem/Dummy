...
export default function ChatListNavbar({ uiBounce, chatContainerRef, themeIndex = 0 }: { uiBounce: boolean; chatContainerRef?: React.RefObject<any>; themeIndex?: number }): JSX.Element {
  ...
  return (
    <div className={themeNavbar(themeIndex, `${uiBounce ? "animate__slideOutUp" : "animate__slideInDown"} shadow-md px-5 h-[60px] flex items-center justify-between py-1 relative rounded-t-3xl ${!convId ? "" : ""}`)}>
      ...
      <div className="flex items-center gap-2.5">
        ...
        {user.role === "admin" || user.role === "superadmin" ? (
          <Link to={APP_ROUTES.ADMIN_DASHBOARD} >
            <TbUserStar className={themeNavbarIcon(themeIndex, "size-[22px] cursor-pointer")} />
          </Link>
        ) : null}
        ...
        <Popover>
          <PopoverTrigger asChild>
            <button>
              <UserPlus className={themeNavbarIcon(themeIndex, "size-[22px] cursor-pointer")} />
            </button>
          </PopoverTrigger>
        ...
        {user.role === "teacher" ? (
          <Popover>
            <PopoverTrigger asChild>
              <button>
                <GraduationCap className={themeNavbarIcon(themeIndex, "size-6")} />
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-auto z-[110] p-0 bg-transparent border-transparent">
