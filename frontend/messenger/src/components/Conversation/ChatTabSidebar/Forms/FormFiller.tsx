...
const FormFiller = ({ assignment, onClose }: any): JSX.Element => {
  ...
  const [answers, setAnswers] = useState<Record<string, any>>(() => {
    ...
    fields.forEach((f: any) => {
      map[f.id] = { value: f.type === "yes_no" ? "yes" : "", explanation: "" };
    });
    return map;
  });
  ...
  const handleSubmit = async () => {
    ...
    const payload = fields.map((f) => ({
      ...
      explanation: answers[f.id].explanation || "",
    }));
              ...
              {field.type === "yes_no" && (
                <div className="pl-8 space-y-3">
                  <div className="flex gap-3">
                    <button
                      onClick={() => updateAnswer(field.id, "value", "yes")}
                    ...
                    <button
                      onClick={() => updateAnswer(field.id, "value", "no")}
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-colors ${
                        ans.value === "no"
                  ...
                  {ans.value === "no" && (
                    <div className="space-y-1">
                      ...
                      <Textarea
                        ...
                        onChange={(e) =>
                          updateAnswer(field.id, "explanation", e.target.value)
                        }
