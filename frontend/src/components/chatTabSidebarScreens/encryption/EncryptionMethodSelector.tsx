...
const EncryptionMethodSelector = ({ encryptionMethod, onMethodChange }: { encryptionMethod: string; onMethodChange: (method: string) => void }): JSX.Element => {
  ...
  const getEncryptionMethodInfo = (method: string) => {
    switch (method) {
      case 'Backend': return {
        ...
        badge: 'Recommended', badgeClass: 'bg-green-500/20 text-green-400 border-green-500/30'
      };
      case 'ECDH': return {
        ...
        badge: 'Advanced', badgeClass: 'bg-blue-500/20 text-blue-400 border-blue-500/30'
      };
      case 'V1': return {
        ...
        badge: 'Legacy', badgeClass: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30'
      };
      default: return { title: '', description: '', badge: '', badgeClass: '' };
    }
  };
  ...
  const handleMethodChange = (method: string) => {
    ...
    if (method === 'Backend') {
      ...
      if (confirmed) onMethodChange(method);
    }
  };
          ...
          <div className="mt-4">
            <Alert className="bg-gray-900 border-gray-700">
              ...
              <AlertTitle className="flex items-center gap-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full border ${methodInfo.badgeClass}`}>{methodInfo.badge}</span>
              </AlertTitle>
              <AlertDescription className="text-gray-300">{methodInfo.description}</AlertDescription>
            </Alert>
