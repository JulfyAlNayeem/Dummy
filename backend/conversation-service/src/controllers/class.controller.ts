...
function formatClass(cls: any) {
  const profile = cls.classProfile ?? {};
  return {
    ...cls,
    _id: cls.id,
    settings: {
      ...
      fileSendingAllowed: profile.fileSendingAllowed,
...
export const updateClass = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (startTime !== undefined || cutoffTime !== undefined) {
...
export const updateClassSettings = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (Object.keys(data).length > 0) {
      await prisma.classProfile.upsert({
        where: { conversationId: classId },
        create: { conversationId: classId, ...data },
        update: data,
      });
      (cls as any).classProfile = { ...(cls as any).classProfile, ...data };
    }
