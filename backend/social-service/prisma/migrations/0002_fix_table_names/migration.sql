-- Migration 0002: Create social tables with correct names (IF NOT EXISTS)
-- This is a safety migration that handles broken production state from 0001_init.
-- If 0001_init ran successfully, these CREATE TABLE IF NOT EXISTS statements are no-ops.
-- FK constraints are NOT added here (already exist from 0001 on fresh installs).
...
CREATE TABLE IF NOT EXISTS `social_post_comment_replies` (
    `id` VARCHAR(191) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `social_page_likes` (
    `id` VARCHAR(191) NOT NULL,
...
-- New social-prefixed tables — won't conflict with api-service
...
-- AddForeignKeys (only if not already exist — use IF NOT EXISTS via try/ignore)
